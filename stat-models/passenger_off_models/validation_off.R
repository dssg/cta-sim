...
# Command Line: Rscript validation_off.R /home/wdempsey/dssg-cta-project/stat-models/passenger_off_models/mcmc_output/avgsim_off.csv 6 0 1423
...
for (k in 1:length(weekend)) {
    ...
    log_p = as.numeric(alpha[obs_halfhour])+as.numeric(gamma[obs_month])+as.numeric(beta[obs_weekend])

    guess = exp(log_p) * pass_in[k]
    truth = test_data$passengers_off[k]
